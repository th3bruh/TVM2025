...
import {Emittable, Emitter} from './emitter';
...
// const readVarInt7 = (byte: uint8): int7 => (byte < 64) ? byte :  -(128 - byte)
...
abstract class val_atom<T> implements Atom<T> {
  readonly t: TypeTag
  readonly z: uint32
  readonly v: T

  constructor(t: TypeTag, z: uint32, v: T) { this.t = t; this.z = z; this.v = v }
  abstract emit(e: Emitter): Emitter // override in subclasses
}
...
export const c = {
  ...
  str_ascii(text: string) {
    ...
    for (let i = 0, L = text.length; i != L; ++i) {
      ...
      if (!!(c & ~0xff))
        throw "ASCII conversion error: char '" + c + "' is outside of ASCII range";
      bytes[i] = 0xff & c;
    }
    return str(bytes)
  },
  ...
  func_type_m(paramTypes: ValueType[], returnTypes: ValueType[]) {
    const paramLen = varuint32(paramTypes.length);
    const resultLen = varuint32(returnTypes.length);
    return new cell(T.func_type, 
      [Func, paramLen, ...paramTypes, resultLen, ...returnTypes]) as any as FuncType
  },
